...
def batch_soft_dtw(X, Y, gamma, metric="L2"):
    ...
    def body(idx, array):
        ...
        def inner_func():
            ...
            r_value = delta_array.read((i-1)*T2+(j-1)) + tf.cast(soft_min_value, tf.float64)
    ...
    def grad(dy):
        ...
        e_matrix = tf.reshape(e_matrix, [N, T1+1, T2+1])

        return e_matrix, e_matrix

    return r_matrix, grad
